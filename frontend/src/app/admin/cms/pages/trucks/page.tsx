...
interface PageSection {
  id: string;
  name: string;
  type: 'hero' | 'featured' | 'comparison' | 'process' | 'cta' | 'ecosystems';
  enabled: boolean;
...
const DEFAULT_SECTIONS: PageSection[] = [
  {
    ...
    config: { heading: 'Built For Every Journey.\nEngineered For Every Ecosystem.', subheading: 'From adventure-ready overland builds to commercial fleet solutions, discover Toyota Hilux trucks designed to perform in every environment and every industry.', backgroundImage: 'https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498050/jp-distribution/home/hero/cinematic-hilux-hero.jpg' },
    selectedTruckIds: [],
  },
  {
    id: 'ecosystems', name: 'Mobility Ecosystems', type: 'featured', enabled: true,
    config: { 
      heading: 'Mobility Ecosystems', 
      subheading: 'We engineer purpose-built Hilux platforms tailored for specific industries, rigorous demands, and extreme environments.',
      items: [
        { 
          id: 1, title: "Adventure & Overland", 
          desc: "Engineered for remote exploration and self-sustained travel across extreme terrain.", 
          icon: "Globe", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498050/jp-distribution/home/hero/cinematic-hilux-hero.jpg",
        ...
        { 
          ...
          icon: "Package", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498078/jp-distribution/trucks/cards/truck-2.png",
        ...
        { 
          ...
          icon: "Truck", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498080/jp-distribution/trucks/cards/truck-3.png",
        ...
        { 
          ...
          icon: "Wrench", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498081/jp-distribution/trucks/cards/truck-4.png",
        ...
        { 
          ...
          icon: "Target", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498040/jp-distribution/custom-builds/cards/build-1.png",
        ...
        { 
          ...
          icon: "Settings", img: "https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498042/jp-distribution/custom-builds/cards/build-2.png",
  ...
  {
    id: 'featured', name: 'Featured Collections', type: 'featured', enabled: true,
    config: { heading: 'Featured Collections', subheading: 'Browse our curated selection of premium Toyota Hilux builds ready for immediate deployment or global export.' },
    selectedTruckIds: [],
  },
  {
    id: 'cta', name: 'Call To Action', type: 'cta', enabled: true,
    config: { heading: 'READY TO IMPORT, BUILD, OR SOURCE?', subheading: 'Partner with a trusted Toyota Hilux import and parts specialist committed to quality, transparency, and your success.' },
    selectedTruckIds: [],
  },
];

export default function TrucksPageEditor() {
  return (
    <Suspense fallback={<div className="min-h-screen text-white p-8 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-red-500" /></div>}>
      <TrucksPageEditorContent />
    </Suspense>
  );
}

function TrucksPageEditorContent() {
  ...
  const fetchPageConfig = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/pages/trucks`);
      const data = await res.json();
      if (data.success && data.data?.sections?.length > 0) {
        const filtered = data.data.sections.filter((s: PageSection) => s.id !== 'comparison' && s.id !== 'process');
  ...
  return (
    <div className="min-h-screen text-white p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 text-xs text-zinc-500 uppercase tracking-widest mb-1">
            <FileText size={12} /> Webpages / Trucks Page
          </div>
          <h1 className="text-2xl font-bold">Trucks Page Editor</h1>
          <p className="text-zinc-400 text-sm mt-1">Configure sections, content, and select which trucks appear on the public Trucks page.</p>
        </div>
        <div className="flex items-center gap-3">
          <a href="/trucks" target="_blank" rel="noopener noreferrer">
      ...
      <div className="bg-blue-950/20 border border-blue-900/40 rounded-lg p-4 mb-8 text-blue-200 text-sm flex items-center gap-2">
        ...
        <span><strong>Trucks Page Media Requirements:</strong> Hero Background &rarr; 16:9 Image. Ecosystem & Featured Cards &rarr; 4:3 Image ratio.</span>
      </div>
        ...
        {(() => {
          ...
          <div className="w-full pb-12">
            ...
            <div className="space-y-10">
                ...
                {/* Truck Selector (only for featured/comparison sections) */}
                {(section.type === 'featured' || section.type === 'comparison') && (
                  <div>
