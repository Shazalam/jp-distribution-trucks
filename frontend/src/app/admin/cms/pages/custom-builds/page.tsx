...
interface PageSection {
  id: string;
  name: string;
  type: 'hero' | 'featured' | 'cta' | 'process';
  enabled: boolean;
...
const DEFAULT_SECTIONS: PageSection[] = [
  {
    ...
    config: { heading: 'Built Beyond Boundaries.', subheading: 'Transforming Toyota Hilux trucks into purpose-built machines engineered for adventure, business, utility, and extreme environments.', backgroundImage: 'https://res.cloudinary.com/dd8a5dpnh/image/upload/f_auto,q_auto/v1781498050/jp-distribution/home/hero/cinematic-hilux-hero.jpg' },
    selectedTruckIds: [],
  },
  {
    id: 'process', name: 'Build Process', type: 'process', enabled: true,
    config: { 
      heading: 'Our Build Process', 
      subheading: 'From concept to completion.',
      items: [
        { id: 1, title: "Consultation", desc: "We discuss your exact needs." }
      ]
    },
    selectedTruckIds: [],
  },
  {
    id: 'featured', name: 'Featured Builds', type: 'featured', enabled: true,
    config: { heading: 'Featured Builds', subheading: 'Our most recent custom projects.' },
    selectedTruckIds: [],
  },
  {
    id: 'cta', name: 'Call To Action', type: 'cta', enabled: true,
    config: { heading: 'Start Your Build', subheading: 'Contact our team to begin designing your custom Toyota Hilux.' },
    selectedTruckIds: [],
  },
];

export default function CustomBuildsPageEditor() {
  return (
    <Suspense fallback={<div className="min-h-screen text-white p-8 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-red-500" /></div>}>
      <CustomBuildsPageEditorContent />
    </Suspense>
  );
}

function CustomBuildsPageEditorContent() {
  ...
  const fetchPageConfig = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE}/pages/custom-builds`);
      const data = await res.json();
  ...
  return (
    <div className="min-h-screen text-white p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 text-xs text-zinc-500 uppercase tracking-widest mb-1">
            <FileText size={12} /> Webpages / Custom Builds
          </div>
          <h1 className="text-2xl font-bold">Custom Builds Page Editor</h1>
          <p className="text-zinc-400 text-sm mt-1">Configure sections, content, and layout for the Custom Builds page.</p>
        </div>
        <div className="flex items-center gap-3">
          <a href="/custom-builds" target="_blank" rel="noopener noreferrer">
      ...
      <div className="bg-blue-950/20 border border-blue-900/40 rounded-lg p-4 mb-8 text-blue-200 text-sm flex items-center gap-2">
        ...
        <span><strong>Custom Builds Media Requirements:</strong> Hero Background &rarr; 16:9 Image. Process & Featured Cards &rarr; 4:3 Image ratio.</span>
      </div>
            ...
            <div className="space-y-10">
                ...
                {section.type === 'featured' && (
                  <div>
                    ...
                    {/* All items grid picker placeholder for Custom Builds */}
                    <div className="p-4 border border-zinc-700/50 bg-zinc-800/30 rounded-xl text-center">
                      <p className="text-zinc-400 text-sm">To select specific custom builds, please ensure the Custom Builds inventory API is connected.</p>
                    </div>
                  </div>
                )}
                ...
                {section.type === 'process' && (
                  <div>
                    <div className="flex items-center justify-between mb-3 border-t border-zinc-800 pt-5 mt-5">
                      <h4 className="text-xs uppercase tracking-widest text-zinc-500 font-bold flex items-center gap-2">
                        <ImageIcon size={14} /> Process Steps
                      </h4>
